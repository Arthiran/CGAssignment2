...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::Checkbox("Use Diffuse Ramp", &useDiffuseRamp)) {

			}
			if (ImGui::Checkbox("Use Specular Ramp", &useSpecularRamp)) {

			}
			if (ImGui::Checkbox("TOGGLE Warm Colour Grading", &toggleWarmGrading)) {
				activeEffect = 2;
				ambientSpecularAndBloom = false;
				ColourCorrectionEffect* temp = (ColourCorrectionEffect*)effects[activeEffect];
				temp->SetEffectCube(warmCube);
				toggleCoolGrading = false;
				toggleCustomGrading = false;
			}
			if (ImGui::Checkbox("TOGGLE Cool Colour Grading", &toggleCoolGrading)) {
				activeEffect = 2;
				ambientSpecularAndBloom = false;
				ColourCorrectionEffect* temp = (ColourCorrectionEffect*)effects[activeEffect];
				temp->SetEffectCube(coolCube);
				toggleWarmGrading = false;
				toggleCustomGrading = false;
			}
			if (ImGui::Checkbox("TOGGLE Custom Colour Grading", &toggleCustomGrading)) {
				activeEffect = 2;
				ambientSpecularAndBloom = false;
				ColourCorrectionEffect* temp = (ColourCorrectionEffect*)effects[activeEffect];
				temp->SetEffectCube(customCube);
				toggleWarmGrading = false;
				toggleCoolGrading = false;
			}

			if (!toggleWarmGrading && !toggleCoolGrading && !toggleCustomGrading && activeEffect == 2)
			{
				ColourCorrectionEffect* temp = (ColourCorrectionEffect*)effects[activeEffect];
				temp->SetEffectCube(neutralCube);
			}
		...
		legoblock1->Set("s_DiffuseRamp", diffuseRamp);
		legoblock1->Set("s_SpecularRamp", specularRamp);
		...
		legoblock2->Set("s_DiffuseRamp", diffuseRamp);
		legoblock2->Set("s_SpecularRamp", specularRamp);
		...
		legocharacter1->Set("s_DiffuseRamp", diffuseRamp);
		legocharacter1->Set("s_SpecularRamp", specularRamp);
		...
		legocharacter2->Set("s_DiffuseRamp", diffuseRamp);
		legocharacter2->Set("s_SpecularRamp", specularRamp);
		...
		legocharacter3->Set("s_DiffuseRamp", diffuseRamp);
		legocharacter3->Set("s_SpecularRamp", specularRamp);
		...
		legocharacter4->Set("s_DiffuseRamp", diffuseRamp);
		legocharacter4->Set("s_SpecularRamp", specularRamp);
		...
		legocharacter5->Set("s_DiffuseRamp", diffuseRamp);
		legocharacter5->Set("s_SpecularRamp", specularRamp);
